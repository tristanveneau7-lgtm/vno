...
export function useCanContinue(step: number): boolean {
  ...
  switch (step) {
    ...
    case 5:
      // All three uploads AND both photo orientations tagged. Orientation is a
      // deliberate tap from the user, not a default — we force the choice here
      // so the engine never guesses and mis-places a portrait into a wide hero.
      return (
